...
class ALlJobs extends Component {
  ...
  onGetJobDetails = async () => {
    ...
    const responseJobs = fetch(jobsApiUrl, optionJobs)
    if (responseJobs.ok === true) {
      ...
      this.setState({
        apiJobStatus: apiJobsStatusConstants.failure,
      })
    }
  }
  ...
  onGetInputOption = event => {
    ...
    const inputNotIntList = checkboxInputs.filter(
      eachItem => eachItem === event.target.id,
    )
    if (inputNotIntList === 0) {
      this.setState(
        prevState => ({
  ...
  onGetJobFailure = () => (
    <div className="failure-mg-button-container">
      <img className="failure-img" src={failureViewImg} alt="failure view" />
      <h1 className="failure-heading">Oops! Something Went Wrong</h1>
      <p className="failure-paragraph">
        we cannot seem to find the page that ypu are looking for.
      </p>
